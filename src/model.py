...
class GatedMultiHeadAttention(nn.Module):
    def __init__(
        ...
        if self.gated_attn:
            self.gate_proj = nn.Linear(self.d_query, self.d_query, bias=True)
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor) -> Any:
        ...
        attn_out = attn_out.transpose(1, 2).contiguous().view(batch_size, q_seq_len, self.d_query)
...
class AttentionBlock(nn.Module):
    def __init__(
        ...
        self.attn_ln = nn.RMSNorm(d_query)
        self.ffn_ln = nn.RMSNorm(d_query)

        self.attn_drop = nn.Dropout(dropout_p)
        self.ffn_drop = nn.Dropout(dropout_p)

        self.kv_ln = nn.Identity() if is_self_attn else nn.RMSNorm(self.d_kv)
...
class AttnRes(nn.Module):
    ...
    def forward(self, history_list):
        ...
        alpha = F.softmax(scores / self.tau, dim=1).unsqueeze(-1) 
...
class FTTBackboneNet(nn.Module):
    ...
    def forward(self, query):
        ...
        for self_block, intersample_block,attnres_layer in zip(self.self_blocks, self.intersample_blocks, self.attnres_layers):
            Q_base = attnres_layer(history)
            self_attn_out = self_block(query=Q_base, key=Q_base, value=Q_base)
            intersample_attn_out = intersample_block(self_attn_out)
            history.append(intersample_attn_out)

        return history[-1]

class BottleneckIntersampleAttention(nn.Module):
    def __init__(self, d_model, num_features, d_latent = None, num_latent_tokens = 8, num_heads=2, dropout_p=0.1, gated_attn = False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x_permuted = x_features.permute(1,0,2).contiguous()
        ...
        latent_attn =  self.self_latent_block(
        ...
        out_features = x_features + self.gamma * intersample_out

        out = torch.cat([cls_token, out_features], dim=1)
        return out


class FTTModel(nn.Module):
    def __init__(self, in_dim, out_dim, d_model, num_latent_tokens = 16, num_self_attn_layers=2, num_heads=4, dropout_p=0.1, gated_attn = False, ):
        super().__init__()
        ...
        self.cls_norm = nn.RMSNorm(d_model)
    ...
    def forward(self, x_num, x_cat, tokenizer):
        ...
        cls_token_out = self.cls_norm(cls_token_out)
